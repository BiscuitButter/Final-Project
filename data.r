...
clean_flow <- function(flow) {
  names(flow) <- c("state_from", "county_from", "state_to", "county_to", "state_name", "county name", "returns", "exempt", "income")
...
write.table(flow, "flow.clean14-18.csv", sep = ",", row = F)
write.table(total, "total.clean14-18.csv", sep = ",", row = F)